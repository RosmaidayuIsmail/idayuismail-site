...
import { rowToLearning } from './index'
...
export default defineEventHandler(async (event) => {
  ...
  if (event.method === 'GET') {
    const result = await portfolioDb.execute({ sql: 'SELECT * FROM learning WHERE slug = ?', args: [slug] })
    if (result.rows.length === 0) {
      setResponseStatus(event, 404)
      return { error: 'Entry not found.' }
    }
    return rowToLearning(result.rows[0])
  }
  ...
  if (event.method === 'DELETE') {
    const authErr = portfolioAuthError(event)
    if (authErr) return authErr
    await portfolioDb.execute({ sql: 'DELETE FROM learning WHERE slug = ?', args: [slug] })
    return { ok: true }
  }
